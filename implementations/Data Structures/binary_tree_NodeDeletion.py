...
class BinaryTreeNode:
    ...
    def delete(self, val): # big O(log n) time complexity because we are traversing only one path to find the element to be deleted
        if val < self.data:
            ...
            # find the minimum element in the right subtree and replace the current node with that element, 
            # then delete the minimum element in the right subtree until the base case is reached where the minimum element is deleted
            min_val = self.right.find_min() 
            self.data = min_val
            self.right = self.right.delete(min_val) # big O(log n) time complexity because we are traversing the tree to find the minimum element in the right subtree
